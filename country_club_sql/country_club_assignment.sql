...
SELECT ROUND( monthlymaintenance * 0.2, 1 ) AS 20_per, membercost, facid, name
FROM 'Facilities' 
WHERE membercost < 20_per;
...
SELECT f.name, CONCAT( m.surname, ', ', m.firstname ) AS full_name
FROM `Bookings` b
...
SELECT CONCAT(sub.fac_name, ', ', m.firstname) AS facility_member,
	   cost
	FROM `Members` m
	JOIN (
           SELECT CASE WHEN b.memid = 0 THEN f.guestcost * book.slots
                       WHEN b.memid != 0 THEN f.membercost * book.slots
        	            END AS cost,
                  f.name as fac_name,
                  b.memid as memid,
...
SELECT sub.fac_name AS facility_name,
	...
	FROM Members m
	JOIN (
           SELECT CASE WHEN b.memid = 0 THEN f.guestcost * b.slots
